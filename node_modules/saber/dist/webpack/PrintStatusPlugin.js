...
module.exports = class PrintStatusPlugin {
    ...
    apply(compiler) {
        compiler.hooks.invalid.tap('show-rebuild-reason', file => {
            const d = new Date();
            log.info(colors.dim(`(${d.getHours()}:${d.getMinutes()}:${d.getSeconds()}) Rebuilding ${this.type} due to changes in ${file.replace(os.homedir(), '~')}`));
        });
        if (this.api.opts.progress !== false &&
            !process.env.CI &&
            ...
            const progressPlugin = new ProgressPlugin((per, message, ...args) => {
                const msg = per === 1
                    ...
                    : `${(per * 100).toFixed(2)}% ${message} ${args
                        ...
                        return message.length > 40
                            ? `...${message.substr(message.length - 39 - this.type.length - 2)}`
                            : message;
                    })
                        .join(' ')}`;
                ...
                const messages = [...progressLogs.keys()]
                    ...
                    .map(type => `[${colors.whiteBright(type)}] ${progressLogs.get(type)}`);
        ...
        compiler.hooks.done.tap('print-status', stats => {
            ...
            if (stats.hasErrors() || stats.hasWarnings()) {
                ...
                log.success(`Compiled ${this.type} successfully in ${prettyTime(Date.now() - stats.startTime)}!`);
                // Only show URL for client build
                if (this.api.dev && this.type === 'client') {
                    const host = this.api.config.server.host === '0.0.0.0'
                        ? 'localhost'
                    ...
                    log.info(`Available at ${colors.underline(`http://${host}:${this.api.actualServerPort}`)}`);
