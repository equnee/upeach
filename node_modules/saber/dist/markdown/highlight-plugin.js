...
const generateLineNumbers = code => '<span aria-hidden="true" class="saber-highlight-line-numbers">' +
    code
        .trim()
        .split('\n')
        .map(() => `<span></span>`)
        .join('') +
    '</span>';
module.exports = (md, { highlightedLineBackground, lineNumbers = false } = {}) => {
    const renderPreWrapper = ({ preWrapperAttrs, preAttrs, codeAttrs, code, codeMask = '', lines = '' }) => `<div${preWrapperAttrs}>${codeMask}<pre${preAttrs}><code${codeAttrs}>${lines}${code.trim()}</code></pre></div>`;
    md.renderer.rules.fence = (...args) => {
        ...
        const highlightLines = fenceOptions.highlightLines
            ? fenceOptions.highlightLines.map(v => `${v}`.split('-').map(v => parseInt(v, 10)))
            : [];
        const codeMask = highlightLines.length === 0
            ? ''
