...
module.exports = async (renderer, { url, hooks, isProd }) => {
    ...
    const documentData = hooks.getDocumentData.call(getInitialDocumentData(context), context);
    // Get document string
    let document = hooks.getDocument.call(getInitialDocument(documentData), context);
    if (isProd) {
        // Remove whitespaces
        document = document.replace(/^\s+/gm, '').replace(/\n+</g, '<');
    }
    return {
        html: `<!DOCTYPE html>${document}`.replace('<div id="_saber"></div>', context.markup),
        context
    };
};
