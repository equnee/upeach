...
class VueRenderer {
    init(api) {
        ...
        this.api.hooks.chainWebpack.tap(ID, (config, { type }) => {
            ...
            if (api.hasDependency('vue')) {
                config.resolve.alias.set('vue', path.dirname(api.localResolve('vue/package.json')));
            }
            else {
            ...
            const vueLoaderOptions = this.hooks.getVueLoaderOptions.call(Object.assign({
                compilerOptions: {
                    modules: []
                },
                transformAssetUrls: {},
            ...
            const pageLoaderOptions = {
                ...
                getTransformerByContentType: contentType => api.transformers.get(contentType),
    ...
    async writeRoutes() {
        ...
        const routesFromPages = pages
            ...
            const chunkNameComment = `/* webpackChunkName: "page--${page.internal.isFile
                ? path
        ...
        const redirectRoutesInBrowser = [...this.api.pages.redirectRoutes.values()]
            ...
            .map(route => `{ path: '${route.fromPath}', redirect: '${route.toPath}' }`)
            .join(',\n');
        const routes = `
    export default [
      ...
      // An addtional route to catch all other requests, i.e. 404 page
      {
        ...
        name: 404,
        component: function () {
          return import(/* webpackChunkName: "404-page" */ ${JSON.stringify(slash(resolveVueApp('404.vue')))})
        }
      }
    ]`;
    ...
    async renderPageContent(url, { scoped = false } = {}) {
        ...
        const content = html.slice(html.indexOf(startingMark) + startingMark.length, html.indexOf(endingMark));
        return scoped
            ? content
    ...
    async generate() {
        ...
        const serverBundle = readJSON(this.api.resolveCache('bundle-manifest/server.json'));
        const clientManifest = readJSON(this.api.resolveCache('bundle-manifest/client.json'));
         ...
         */
        const writeFiles = routes => Promise.all(routes.map(async (route) => {
        ...
        await fs.copy(this.api.resolveCache('dist-client'), path.join(outDir, '_saber'));
    ...
    getRequestHandler() {
        ...
        this.api.compilers.server.on('status-changed', ({ status }) => {
            if (status === 'success') {
                serverBundle = readJSON(this.api.resolveCache('bundle-manifest/server.json'), mfs.readFileSync.bind(mfs));
                this.initRenderer({ serverBundle, clientManifest });
            }
        });
        this.api.compilers.client.on('status-changed', ({ status }) => {
            if (status === 'success') {
                clientManifest = readJSON(this.api.resolveCache('bundle-manifest/client.json'), clientCompiler.outputFileSystem.readFileSync.bind(clientCompiler.outputFileSystem));
                this.initRenderer({ serverBundle, clientManifest });
            }
        });
      ...
      `);
        ...
        server.use(require('serve-static')(this.api.resolveCwd('static'), {
            dotfiles: 'allow'
        }));
        server.use(require('serve-static')(path.join(this.api.theme, 'static'), {
            dotfiles: 'allow'
        }));
